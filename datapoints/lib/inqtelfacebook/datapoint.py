...
class DataPoint(BaseDataPoint):
    def get_unconfigured_config(self):
        return {
            'type':'inqtelfacebook',
            'sub_type':'inqtelfacebook',
            'is_live':True,
            'display_name_short':'Facebook',
            'full_display_name':'Facebook Search',
            'instructions':'',
    ...
    def get_content_item_template(self):
        return ""\
                "<li style='width:100%;'>"\
                    "<a href='${author_link}'>"\
                        "<img src='${author_image}' style='width:50px; padding:1px; box-shadow: 3px 3px 3px #111;' align='left' class='helper_corner tool_tip' title='<b>${author_display_name}</b> - click to view their profile on Facebook' />" \
                    "</a>"\
                    ...
                    "<img src='/static/images/thedashboard/data_points/facebook_small.png' style='width:15px;'/>"\
                    "</p>"\
                    "<a href='${author_link}' target='_blank'><p style='padding-left:60px;' class='tool_tip' title='click to see original post on Facebook'>${display_text_abstract(strip_html(title))}</p></a>"\
                    "<ul style='padding-left:60px;' class='actions'>" \
                    "</ul>"\
                "</li>"

    def generate_configured_guid(self, config):
        base_string = 'inqtelfacebook_source'
        return md5(base_string).hexdigest()

    def generate_configured_display_name(self, config):
	return 'Facebook: North Korea'
